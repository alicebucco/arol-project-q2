...
`X-Is-Truncated`, and `X-Limit` expose completeness. The frontend is unchanged.
...
The Service Agent does not retrieve manuals, telemetry aggregates, maintenance
requirements, or maintenance-due conclusions. The orchestrator correlates the
separate IoT and Manuals evidence when a maintenance observation is requested.
