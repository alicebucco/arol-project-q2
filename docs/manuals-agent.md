...
Before using model-generated manual claims, an orchestrator should call
...
entailment from natural language; the orchestrator must still use cautious wording
and discard rejected claims.
...
that maintenance is due. A new orchestrator combines this documentary evidence
with `iot.observed_productive_hours()` and `service.search_tickets()`.
