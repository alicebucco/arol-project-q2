...
from api import auth, chat, commercial, machines, manuals, operations
import main
...
def test_ticket_api_filters_metadata_and_detail(client, monkeypatch):
    ...
    monkeypatch.setattr(operations, "search_tickets", search)
    monkeypatch.setattr(operations, "ticket_detail", AsyncMock(return_value=row))
...
def test_ticket_api_invalid_filter_and_missing_detail(client, monkeypatch):
    ...
    monkeypatch.setattr(operations, "ticket_detail", AsyncMock(side_effect=TicketNotFoundError("TCK-1")))
