...
from main import manual_search_result
...
def test_public_result_exposes_precise_chunk_provenance() -> None:
    result = manual_search_result(
        {
            "source": "manual",
            "chunk_id": "15610-p42-3",
            "file": "15610_manual_EN.pdf",
            "page": 42,
            "section": "mechanical",
            "excerpt": "The complete indexed chunk.",
            "title": "Mechanical procedure",
            "highlights": ["pressure"],
            "relevance": 0.72,
            "similarity": 0.61,
            "similarity_threshold_met": True,
            "alarm_code_match": "semantic_only",
            "excerpt_is_complete_chunk": True,
            "section_category": "mechanical",
            "section_category_is_inferred": True,
            "documented_section_title": None,
        }
    )

    assert result.citation.chunk_id == "15610-p42-3"
    assert result.excerpt_is_complete_chunk is True
    assert result.section_category == "mechanical"
    assert result.documented_section_title is None
...
def test_claim_validation_rebuilds_citations_from_authorized_evidence() -> None:
    ...
    result = validate_manual_claim_links(
        [
            ...
            {
                "claim": "The manual documents a procedure.",
...
def test_maintenance_requirements_keep_source_text_and_skip_contents_pages() -> None:
    chunks = [
        {
            "source": "manual", "chunk_id": "p6", "file": "manual.pdf", "page": 6,
            "section": "general",
            "content": "EVERY 500 WORKING HOURS ........ 90 EVERY 1000 WORKING HOURS ........ 91",
        },
        {
            "source": "manual", "chunk_id": "p90-short", "file": "manual.pdf", "page": 90,
            "section": "mechanical",
            "content": "EVERY 500 WORKING HOURS OPERATION AIM NOTES Lubricate the closure head.",
        },
        {
            "source": "manual", "chunk_id": "p90-full", "file": "manual.pdf", "page": 90,
            "section": "mechanical",
            "content": (
                "EVERY 500 WORKING HOURS OPERATION AIM NOTES Lubricate the closure head. "
                "If wear is observed, replace the affected component."
            ),
        },
        {
            "source": "manual", "chunk_id": "p91", "file": "manual.pdf", "page": 91,
            "section": "mechanical",
            "content": "EVERY 1000 OPERATING HOURS OPERATION AIM NOTES Inspect electrical connections.",
        },
    ]
    ...
    assert requirements[0]["documented_conditions"] == [
        "If wear is observed, replace the affected component."
    ]
...
def test_rerank_distinguishes_exact_code_in_a_passage_from_semantic_evidence() -> None:
    candidates = [
        {
            ...
            "content": "AL017_LOW_AIR_PRESSURE requires inspection.",
            "similarity": 0.70,
        },
        {
            "file": "manual.pdf",
            "page": 13,
            "section": "troubleshooting",
            "content": "Inspect the pneumatic pressure circuit.",
            "similarity": 0.65,
        },
    ]
    ...
    assert [result["alarm_code_match"] for result in results] == [
        "exact_in_passage",
        "semantic_only",
    ]


def test_search_with_match_status_keeps_semantic_evidence_when_code_is_not_indexed(
            ...
            return_value=[
                {
                    ...
                    "page": 8,
                    "section": "troubleshooting",
                    "content": "Remove fallen bottles before restarting the line.",
                    "similarity": 0.62,
                }
            ]
        ),
    ...
    result = asyncio.run(
        manuals.search_with_match_status(
            "MCH-1",
            "What does AL083_FALLEN_BOTTLE_ALARM mean?",
            AuthContext("USR-1", "CMP-1", "full"),
        )
    )
