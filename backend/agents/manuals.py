...
def _alarm_codes(query: str) -> list[str]:
    ...
    return sorted({match.group(0).upper() for match in ALARM_CODE_PATTERN.finditer(query)})
...
def validate_manual_claim_links(
    ...
    Each claim must provide `claim`, `chunk_id`, and `supporting_quote`. The
    quote must occur in the full authorized chunk. Returned citations are always
    reconstructed from backend evidence rather than trusting model-provided
    filename, page, or section fields. This validates provenance and quotation;
    it does not prove that a natural-language claim logically follows from the
    quote, so callers must retain cautious answer rules.
    """
    ...
    def select(
        ...
        *,
        ...
        for score, candidate in ranked_candidates:
            ...
            # The index records a category inferred from heading-like text, not
            # a verified heading copied from the PDF.
